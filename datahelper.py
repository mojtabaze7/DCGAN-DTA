...
def vector_seq(line, MAX_SEQ_LEN, pro2vec, emb_dim):
    X = np.zeros(shape=(MAX_SEQ_LEN, emb_dim))
    for i, ch in enumerate(line[:MAX_SEQ_LEN]):
        X[i, :] = pro2vec[ch]
    print(X.shape)
    return X  # .tolist()
...
class DataSet(object):
    ...
    def parse_data(self, FLAGS, with_label=2):
        fpath = FLAGS.dataset_path
        print("Read %s start" % fpath)

        with open(fpath + "protein_feature_vec.json") as f:
            pro2vec = json.load(f)
        if with_label == 1:
            ligands = json.load(open(fpath + "ligands.txt"), object_pairs_hook=OrderedDict)
            proteins = json.load(open(fpath + "protVec.txt"), object_pairs_hook=OrderedDict)
        else:
            ligands = json.load(open(fpath + "ligands.txt"), object_pairs_hook=OrderedDict)
            proteins = json.load(open(fpath + "proteins.txt"), object_pairs_hook=OrderedDict)

            ligands_train = json.load(open(fpath + "ligands_train.txt"), object_pairs_hook=OrderedDict)
            proteins_train = json.load(open(fpath + "proteins_train.txt"), object_pairs_hook=OrderedDict)
        ...
        if with_label == 1:
            ...
            for d in ligands_train.keys():
                XD_t.append(label_smiles(ligands_train[d], self.SMILEN, self.charsmiset))

            for t in proteins.keys():
                XT.append(label_sequence(proteins[t], self.SEQLEN, self.charseqset))

            for t in proteins_train.keys():
                XT_t.append(label_sequence(proteins_train[t], self.SEQLEN, self.charseqset))
                
        elif with_label == 3:
            for d in ligands.keys():
                XD.append(one_hot_smiles(ligands_train[d], self.SMILEN, self.charsmiset))

            for t in proteins.keys():
                XT.append(one_hot_sequence(proteins[t], self.SEQLEN, self.charseqset))

        return XD, XT, Y, XD_t, XT_t
